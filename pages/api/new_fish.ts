...
export default async function handler(
  ...
  if (!user) {
    const userCreationResponse = await client.query(`
      INSERT INTO users (username) VALUES('${username}');
   `).catch(e => { console.log(e); error = "Failed to create a new user for fish insertion" })
   ...
   `).catch(e => {console.log(e); error = "Failed to get new user information"});
